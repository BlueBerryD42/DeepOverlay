...
/**
 * Storage key for a single image/page record under a work.
 * Kept small to avoid hitting Chrome storage per-item limits when notes are long.
 * @param {string} storageKey work:<site>:<workId>
 * @param {string} imageKey pageUrl + sep + cssSelector (or legacy cssSelector)
 */
export function makeWorkImgKey(storageKey, imageKey) {
...
export function migrateStorageSnapshot(all) {
  ...
  for (const key of Object.keys(all)) {
    ...
    if (val && typeof val === 'object' && 'images' in val) {
      ...
      // Split heavy boxes into separate workimg:* keys (schema v4)
      if (we.images && typeof we.images === 'object') {
        ...
        for (const imageKey of Object.keys(nextImages)) {
          ...
          if (img.refKey) continue; // already migrated
          if (!Array.isArray(img.boxes)) continue; // nothing heavy to split
